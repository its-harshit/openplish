...
import type { TaskMessage } from '@somehow_ai/agent-core/common';
...
import { AssistantThinkingCollapsible } from './AssistantThinkingCollapsible';
import { getAssistantMessageDisplayParts } from '@/lib/assistant-message-display';
...
export const MessageBubble = memo(
  function MessageBubble({
    ...
    const assistantParts = isAssistant ? getAssistantMessageDisplayParts(message) : null;
    const bodyText = assistantParts?.body ?? message.content;
    const thinkingText = assistantParts?.thinking ?? null;

    const showCopyButton =
      !isTool &&
      (isUser || isSystem ? !!message.content?.trim() : isAssistant ? !!bodyText.trim() : false);
          ...
          <>
            {isAssistant && thinkingText ? (
              <AssistantThinkingCollapsible
                ...
                inProgress={shouldStream && !streamComplete}
                className="mb-2 max-w-[85%] w-full min-w-0"
              />
            ) : null}
            <div
              className={cn(
            ...
            >
              ...
              {showCopyButton && (
                <MessageCopyButton
                  content={isAssistant ? bodyText : message.content}
                  isUser={isUser}
                />
              )}
            </div>
