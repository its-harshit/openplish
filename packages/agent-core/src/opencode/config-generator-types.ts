...
export interface ConfigGeneratorOptions {
  ...
  skills?: Skill[];
  /**
   * Directories whose immediate subfolders are skills (each containing SKILL.md).
   * Passed to OpenCode as `skills.paths` so the built-in `skill` tool can resolve names.
   */
...
export interface OpenCodeConfigFile {
  ...
  plugin?: string[];
  /** OpenCode: extra skill roots for the native `skill` tool (see https://opencode.ai/config.json) */
