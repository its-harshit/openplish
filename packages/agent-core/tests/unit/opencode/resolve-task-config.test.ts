...
describe('resolveTaskConfig', () => {
  ...
  it('passes skillRootPaths through to configOptions', async () => {
    const storage = createMockStorage();
    const skillRootPaths = ['/bundled-skills', '/user/skills'];
    const result = await resolveTaskConfig({
      storage,
      platform: 'darwin',
      mcpToolsPath: '/tools',
    ...
    expect(result.configOptions.skillRootPaths).toEqual(skillRootPaths);
  });
