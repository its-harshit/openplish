...
describe('ConfigGenerator', () => {
  ...
  describe('generateConfig', () => {
    ...
    it('should set OpenCode skills.paths when skillRootPaths is non-empty', () => {
      const roots = [path.join(userDataPath, 'bundled-skills'), path.join(userDataPath, 'skills')];
      const options: ConfigGeneratorOptions = {
        ...baseOptions,
        mcpToolsPath,
        userDataPath,
        skillRootPaths: roots,
      };

      const result = generateConfig(options);

      expect(result.config.skills?.paths).toEqual(roots);
      const parsed = JSON.parse(fs.readFileSync(result.configPath, 'utf8'));
      expect(parsed.skills.paths).toEqual(roots);
    });
